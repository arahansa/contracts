...
contract('PlayCoin2', function(accounts) {
  ...
  it("only vault can recall", async () => {
  ...
  it("recall fails", async () => {
    ...
    try {
      ...
      await coin.recall(user2, OnePlayCoin, {from: vault});
      assert.fail();
    }
    catch(exception) {
      assert.isTrue(exception.message.includes("revert"));
    ...
    try {
      ...
      await coin.recall(user2, OnePlayCoin * 1.1, {from: vault});
      assert.fail();
    }
    catch(exception) {
      assert.isTrue(exception.message.includes("revert"));
  ...
  it("after recall all coin", async () => {
    ...
    coin.recall(user3, OnePlayCoin, {from: vault});
  ...
  it("after recall half", async () => {
    ...
    coin.recall(user4, OnePlayCoin / 2, {from: vault});
